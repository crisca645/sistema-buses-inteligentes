...
public class ValidatorsService {
    ...
    public boolean validationRolePermission(HttpServletRequest request, String url, String method) {
        ...
        // Permitir endpoints públicos o especiales
        if (url.contains("/security/complete-profile") ||
                url.contains("/security/login") ||
                url.contains("/security/forgot-password") ||
                url.contains("/security/reset-password") ||
                url.contains("/oauth2") ||
                url.contains("/login")) {
            return true;
        }
    ...
    public User getUser(final HttpServletRequest request) {
        ...
        if (theSession == null ||
                theSession.getExpiration() == null ||
                theSession.getExpiration().before(new Date())) {
            return null;
        }
