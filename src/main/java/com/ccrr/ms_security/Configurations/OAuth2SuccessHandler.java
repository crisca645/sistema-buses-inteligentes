...
public class OAuth2SuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        try {
            ...
            redirectToLoginWithError(response, "Error en autenticación OAuth: " + e.getMessage());
        }
    }
    ...
    private void handleGithubLogin(OAuth2AuthenticationToken token,
        ...
        if (client == null || client.getAccessToken() == null) {
            redirectToLoginWithError(response, "No se pudo obtener el access token de GitHub");
            return;
        }
        ...
        if (authResponse == null) {
            redirectToLoginWithError(response, "No se pudo autenticar con GitHub");
            return;
        }

        if (authResponse.isRequiresEmailCompletion()) {
            String pid = authResponse.getPendingProviderId();
            if (pid == null || pid.isBlank()) {
                redirectToLoginWithError(response, "No se pudo obtener el identificador de GitHub");
                return;
            }
            response.sendRedirect(frontendUrl(
                    "/complete-github-email?requiresEmailCompletion=1"
                            + "&providerId=" + encode(pid)
                            + "&username=" + encode(emptyToBlank(authResponse.getPendingUsername()))
                            + "&name=" + encode(emptyToBlank(authResponse.getPendingName()))
                            + "&picture=" + encode(emptyToBlank(authResponse.getPendingPicture()))
            ));
            return;
        }

        if (authResponse.isEmailRequired()) {
            if (authResponse.getToken() == null || authResponse.getToken().isBlank()) {
                redirectToLoginWithError(response, "No se pudo iniciar el flujo de email GitHub");
                return;
            }
            response.sendRedirect(frontendUrl("/complete-github-email?token=" + encode(authResponse.getToken())));
            return;
        }

        User user = authResponse.getUser();
        if (securityService.isProfileIncomplete(user)) {
            if (user == null || user.getId() == null || user.getId().isBlank()) {
                redirectToLoginWithError(response, "No se pudo identificar el usuario para completar perfil");
                return;
            }
            response.sendRedirect(frontendUrl("/complete-profile?userId=" + encode(user.getId())));
            return;
        }
        ...
        if (jwtToken == null || jwtToken.isBlank()) {
            if (user == null) {
                redirectToLoginWithError(response, "No se pudo generar sesión OAuth");
                return;
            }
            jwtToken = securityService.generateTokenForUser(user);
        }

        response.sendRedirect(frontendUrl("/oauth-success?token=" + encode(jwtToken)));
    }
    ...
    private void handleGenericOAuthLogin(OAuth2AuthenticationToken token,
        ...
        if (email == null || email.isBlank()) {
            redirectToLoginWithError(response, "No se pudo obtener el email. Verifica tu cuenta en " + registrationId);
            return;
        }
        ...
        if (user == null) {
            redirectToLoginWithError(response, "Error al crear o encontrar el usuario");
            return;
        }
        ...
        response.sendRedirect(frontendUrl("/oauth-success?token=" + encode(jwt)));
    }

    private void redirectToLoginWithError(HttpServletResponse response, String errorMessage) throws IOException {
        response.sendRedirect(frontendUrl("/login?oauthError=" + encode(errorMessage)));
    }

    private String frontendUrl(String pathAndQuery) {
        if (frontendBaseUrl.endsWith("/")) {
            return frontendBaseUrl.substring(0, frontendBaseUrl.length() - 1) + pathAndQuery;
        }
