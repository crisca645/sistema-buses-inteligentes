...
export class RolePermissionsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    try {
      ...
      this.notify.error(
        this.apiErrorMessage(e, 'No se pudieron cargar roles o el catálogo de permisos'),
      );
    }
  }
  ...
  async onRoleChange(): Promise<void> {
    ...
    try {
      ...
      this.assigned = [];
      this.notify.error(
        this.apiErrorMessage(e, 'No se pudieron cargar los permisos asignados a este rol'),
      );
    } finally {
      this.busy = false;
    }
  ...
  async assign(permission: Permission): Promise<void> {
    ...
    try {
      ...
      this.notify.error(this.apiErrorMessage(e, 'No se pudo asignar el permiso'));
    }
  }
  ...
  async remove(rp: RolePermission): Promise<void> {
    ...
    try {
      ...
      this.notify.error(this.apiErrorMessage(e, 'No se pudo quitar el permiso'));
    }
  }
}
