...
public class SecurityController {
    ...
    public HashMap<String, Object> login(@RequestBody LoginRequest loginRequest,
        ...
        if (!Boolean.TRUE.equals(theResponse.get("success"))) {
            String errorType = theResponse.get("errorType") instanceof String ? (String) theResponse.get("errorType") : "";
            if (SecurityService.ERROR_TYPE_EMAIL_2FA_FAILED.equals(errorType)) {
    ...
    public ResponseEntity<?> completeProfile(@RequestBody Map<String, String> body) {
        ...
        try {
            ...
            HashMap<String, Object> result =
                    theSecurityService.completeProfileAndGenerateToken(userId, address, phone);

            if (result == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(Map.of("message", "Usuario no encontrado o datos inválidos"));
            }
    ...
    @PostMapping("github/register-alternate-email")
    public ResponseEntity<?> registerGithubAlternateEmail(@RequestBody GithubAlternateEmailRequest body) {

        try {
            if (body == null || body.getProviderId() == null || body.getProviderId().isBlank()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(Map.of("message", "providerId es obligatorio"));
            }
            ...
            AuthResponse response = githubAuthService.registerGithubWithAlternateEmail(body);
